...
int clock_gettime(int dummy, struct timespec* ct) {
    ...
    ct->tv_sec = counter.QuadPart / frequency.QuadPart;
    ct->tv_nsec = (long)(((counter.QuadPart % frequency.QuadPart) * 1000000000) / frequency.QuadPart);
    return 0;
}
#endif
...
void BuscaParalela(int num_threads) {
    // Alocação dinâmica em vez de VLA
    ...
    // Liberar a memória alocada no final da função
    free(threads);
}
...
int menu() {
    ...
    printf("3 - Comparar (Apenas apos ambos os benchmarks forem executados)\n");
    printf("0 - Sair\n");
...
ResultadoBenchmark* adicionarResultado(ResultadoBenchmark* resultados, int* n_resultados, int threads, double tempo, int primos) {
    ...
    if (temp == NULL) {
        printf("Erro ao alocar memoria resultados paralelos foram limpos...\n");
        *n_resultados = 0;
        return NULL;
    }
...
int main() {
    ...
    while (opcao != 0) {
        ...
        case 2:
            ...
            int num_threads;
            printf("Digite o numero de threads para a busca paralela: ");
            scanf("%d", &num_threads);
        ...
        case 3:
            ...
            if (resultados_paralelo == NULL || n_resultados == 0 || resultado_serial.tempo == 0.0) {
                printf("Execute a busca serial e paralela primeiro para obter os tempos de referencia.\n");
            }
            else {
